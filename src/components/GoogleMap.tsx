...
const MapComponent = () => {
  ...
  >(undefined);
  const [mapObject, setMapObject] = useState<google.maps.Map | undefined>(
    undefined
  );
  ...
  const [destinationMarker, setDestinationMarker] = useState<
    google.maps.Marker | undefined
  >(undefined);
  const [firstLegPolyLine, setFirstLegPolyLine] = useState<
  ...
  const selectMap = () => {
    if (adaptionContext.uiState.mapType === MapType.TERRAIN) {
      updateMap(
        undefined,
        adaptionContext.uiState.colorTheme === ColorThemes.DARK
          ? "df8bee2c01acfbbc"
          : "8171b9413b7fa18a"
      );
    } else {
  ...
  useEffect(() => {
    ...
    const checkIfUserReachedDestination = async () => {
      const distanceToDestination =
        geolocationContext.getDistanceToDestination();
      ...
      if (distanceToDestination != null && distanceToDestination < 0.01) {
        ...
        const areAdaptionsUploaded =
          await adaptionContext.utility.emitAdaptions();
  ...
  const drawPolylineToFirstWaypoint = (
    userGeodata: google.maps.GeocoderResult[],
    directionsResult: google.maps.DirectionsResult
  ) => {
    const path = [
      {
